...
from postprocessing.chrest.chrestData import ChrestData
from postprocessing.chrest.supportPaths import expand_path
from scipy.spatial import KDTree
import postprocessing.chrest.interpolate
from enum import Enum
from postprocessing.chrest.xdmfGenerator import XdmfGenerator
                    ...
                    for t in range(len(ablate_field_in_chrest_order)):
                        if len(ablate_field_data[f].shape)>2:
                            for ii in range(ablate_field_data[f].shape[2]):
                                ...
                                ablate_field_in_chrest_order[t,:,ii] = np.reshape(
                                    interpolate.interpolate3D(self.vtx, self.wts, fielddata[:, ii], points), (1, self.vtx.shape[0]))
                        else:
                            fielddata=np.transpose(ablate_field_data[f][:])
                            ablate_field_in_chrest_order[t,:] = np.reshape(
                                interpolate.interpolate3D(self.vtx, self.wts, fielddata[:, t], points), (1, self.vtx.shape[0]))
                else:  
                    # closest node 
